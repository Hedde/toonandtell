...
- **Fingerprint-in-de-loop: niets sluit de karakter-vingerafdruk betrouwbaar.** Oppervlakte-chirurgie (komma's/functiewoorden) faalt of verergert. Eén dialoog-vorm-rewrite *leek* de gap te dichten (n=1), maar bij **n=8 dialoog-vorm-kandidaten + best-of-N-selectie repliceert dat niet**: de held-out char-3-gram blijft voor álle AI-verhalen buiten het echte bereik (≥1,05 vs echt ≤0,98), en selectie op MFW transfereert er niet heen (r=0,40). MFW blijkt bovendien een zwakke discriminator. Honest gecorrigeerd: prompt/proces-hefbomen verbeteren de **leesindruk**, niet de diepe (karakter-niveau) vingerafdruk — die persisteert, consistent met de literatuur die modelniveau-ingrepen (fine-tunen) aanwijst. Detail: `stylometry/RESULTS.md` §7-8.
