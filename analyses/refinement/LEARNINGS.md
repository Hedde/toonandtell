...
Slotsom: de enige plek waar we ergens wijzen waar de cited papers niet keken, is de architectuur-/premisse-laag als interventiepunt.

**Update — de stylometrische pilot is nu gedraaid** (`stylometry/RESULTS.md`). Uitkomst: premisse-eerst ligt stylometrisch *niet* dichter bij Tellegen dan thema-eerst (Delta 0,850 vs 0,869; verschil binnen de ruis), en beide modi houden dezelfde meetbare tells (komma-overgebruik +34%, lagere lexicale diversiteit). Het premisse-eerst-effect zit dus in de *leesindruk*, niet in de forensische vingerafdruk. Dit **strookt met** de geciteerde literatuur (surface/architectuur ≠ diepe vingerafdruk) en weerlegt die niet. Het kandidaat-novum verschrompelt daarmee tot: *een reader-impression-strategie*, geen stylometrische verbetering. Plagiaat-audit: verwaarloosbare verbatim-overlap (één stockfrase-collisie eerlijk gemeld).

**Vervolg (RESULTS §7) — wél een stylometrische hefboom, maar via vorm:** een chirurgie/review-lus die oppervlakte-woordkeuze bijstelt faalt (verergert zelfs, held-out char3 onbewogen). Maar de **compositievorm** matchen (de solo-zeekomkommer dialoog-gedreven maken) bracht béíde vingerafdrukken binnen de Tellegen-spreiding (MFW 0,843→0,755; char3 1,007→0,914) — genuine convergentie. De vingerafdruk wordt gedomineerd door dialoogdichtheid/vorm, niet door tics; te dichten in een loop, maar ten koste van vorm-originaliteit (convergeren naar Tellegens dialoog-mal). n=1, twee feature-families.

**CORRECTIE na uitbreiding (n=8 + harness-controle; RESULTS §8):** dit repliceert niet. Bij 8 dialoog-vorm-kandidaten blijft char-3-gram voor álle AI-verhalen buiten het echte per-verhaal-bereik (≥1,05 vs ≤0,98); best-of-N-selectie op MFW transfereert niet naar char3 (r=0,40); en MFW is een zwakke discriminator (AI/echt overlappen). De n=1-"convergentie" was een uitschieter. Eerlijke stand: **geen geteste prompt/proces-hefboom (premisse, vorm, best-of-N) sluit de karakter-niveau-vingerafdruk** — die persisteert, conform de literatuur; alleen modelniveau (fine-tunen) is geïndiceerd.

## 10. Constraints vs surgery — de balans (bewezen; corrigeert §6-8 deels positief)
Iteratieve, data-gedreven surgery helpt één pas (mees: char3 1,140→1,016) en plateaut daarna (r2 1,072) — geen monotone optimizer. Maar het verplaatsen van de **systematische** afwijkingen (gemeten over 16 teksten: was/niet/dan/nu te vaak; in/zijn/uit te zelden; komma-overmaat; lage TTR) naar **register-constraints in de schrijfagent** verschoof de baseline op béíde families binnen het Tellegen-bereik, **inclusief de held-out char-3-gram**: 5 constrained teksten char3 0,82–0,91 vs 1,05–1,30 niet-constrained; confound met diernaam uitgesloten (reiger char3 1,30→0,90, kreeft 1,09→0,83 bij identiek dier). Teksten blijven leesbaar.

**Balans:** systematisch → constraint (groot, goedkoop, generaliseert); idiosyncratisch → hooguit één surgical pass. Dit nuanceert de eerdere "geen prompt/proces-hefboom sluit de gap": op ONZE twee families sluit de constraint-route hem wél. **Terughoudendheid:** 2 families, eigen harness, n=5, één auteur, constraints afgeleid van dezelfde auteur (char3 wel held-out). Geen bewijs voor neurale/perplexiteits-detectoren, menselijke lezers of andere auteurs — generieke toepasbaarheid vergt meer empirisch werk.

## 11. Model-vergelijking (4.8 / 4.6 / 4.5 / Codex) — meetbare indruk
Zelfde taak + constraints, alleen schrijfmodel gevarieerd, N=4/model (4.7 niet beschikbaar). Op de held-out char-3-gram: **Opus 4.8 0,906** (binnen echt bereik) < Codex 1,051 < Haiku 4.5 1,105 < Sonnet 4.6 1,117; op MFW zijn 4.8≈4.6, Codex daarachter, Haiku verst. Dit is de eerste gemeten basis voor de eerder onbewezen "modelbijdrage": 4.8 ligt het dichtst bij de doelstijl op de scherpste vingerafdruk. Codex levert in deze proef geen verbetering t.o.v. Opus 4.8, maar zit op char3 wel vóór Sonnet/Haiku. Voorbehoud: klein (N=4, 2 families, 1 auteur, eigen harness, constraints deels van 4.8-data afgeleid); Codex is bovendien niet blind gemeten omdat schrijver en analistcontext samenvielen. Indruk, geen ranglijst-bewijs.
